...
from core.shared.storage_lifecycle import assess_storage_lifecycle
...
def run_wipe_process(device_path: str, sensitivity: str = "moderate", will_reuse: bool = True, leaves_control: bool = False):
    ...
    try:
        ...
        for dev in devices:
            ...
            # Also handle Windows device paths, virtual devices, and sandbox prefixes
            matches = [
                ...
                # Add support for sandbox prefixes
                (device_path.startswith("sandbox:") and dev_path and device_path.replace("sandbox:", "") == dev_path),
                (device_path.startswith("sandbox:") and dev_name in device_path)
            ]
        ...
        # Check if this is sandbox mode (device path starts with "sandbox:")
        is_sandbox_mode = device_path.startswith("sandbox:")
        if is_sandbox_mode:
            print(f"DEBUG: Sandbox mode detected - will simulate wipe process")
            # Remove sandbox prefix for lifecycle assessment
            real_device_path = device_path.replace("sandbox:", "")
            device_info.path = real_device_path  # Use real path for lifecycle assessment
        else:
            real_device_path = device_path
        ...
        # Assess storage lifecycle (best-effort) - do this first
        try:
            print(f"DEBUG: Assessing lifecycle for device: {device_info.path}")
            lifecycle = assess_storage_lifecycle(device_info.path)
            print(f"DEBUG: Lifecycle result: {lifecycle}")
        except Exception as _e:  # noqa: BLE001
            print(f"DEBUG: Lifecycle assessment failed: {_e}")
            lifecycle = {"available": False, "error": str(_e)}
        ...
        # Add lifecycle data to status for UI display
        wipe_status["storage_lifecycle"] = lifecycle
        print(f"DEBUG: Added lifecycle data to wipe_status: {lifecycle}")
...
def api_reset():
    """API endpoint to reset wipe status for testing."""
    global wipe_status
    wipe_status = {
        "running": False, 
        "output": "", 
    ...
    return jsonify({"status": "reset", "message": "Wipe status reset successfully"})
